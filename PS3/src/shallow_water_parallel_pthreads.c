...
void time_step(int, pthread_barrier_t *);
...
int main(int argc, char **argv)
{
    ...
    pthread_barrier_t thread_barrier;
    pthread_barrier_init(&thread_barrier, NULL, (unsigned)threadcount);
    thread_args t_args[threadcount];
    for (int i = 0; i < threadcount; i++)
    {
        t_args[i].part = i;
        t_args[i].thread_barrier = &thread_barrier;
        pthread_create(&t_args[i].thread_handle, NULL, &thread_work, &t_args[i]);
    }

    for (int i = 0; i < threadcount; i++)
    {
        pthread_join(t_args[i].thread_handle, NULL);
    }
...
void time_step(int part, pthread_barrier_t *thread_barrier)
{
    int start = part * N / threadcount + 1;
    int end = start + N / threadcount - 1;

    for (int_t y = start; y <= end; y++)
        for (int_t x = 1; x <= N; x++)
        {
    ...
    if (part == threadcount - 1)
    {
        for (int_t y = start; y <= N + 1; y++)
            for (int_t x = 0; x <= N + 1; x++)
            {
                DU(y, x) = PN(y, x) * U(y, x) * U(y, x) + 0.5 * gravity * (PN(y, x) * PN(y, x) / density);
                DV(y, x) = PN(y, x) * V(y, x) * V(y, x) + 0.5 * gravity * (PN(y, x) * PN(y, x) / density);
...
void *thread_work(void *data)
{
    ...
    for (int_t iteration = 0; iteration <= max_iteration; iteration++)
    {
        if (args->part == 0)
        {
            ...
            pthread_barrier_wait(args->thread_barrier);
        }
        else
        {
            pthread_barrier_wait(args->thread_barrier);
        ...
        time_step(args->part, args->thread_barrier);

        pthread_barrier_wait(args->thread_barrier);

        if (iteration % snapshot_frequency == 0 && args->part == 0)
        {
            printf(
